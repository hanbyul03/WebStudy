...
<!-- 
 ...
 : 사용처 : 로그인시에 사용자 정보 저장
 => 동작
    1. 사용자가 JSP에서 처음 접근시에 => 서버는 새로운 세션을 생성
       => 사용자(브라우저)마다 1개 생성
    2. 사용자가 브라우저를 종료 / 일정시간이 지나면 자동 해제
       => 기본 30분
 => 주요 메소드
    setAttribute(String key, Object obj) : 세션에 저장
    gettAttribute(String key) : 저장된 세션값 읽기
    removetAttribute(String key) : 특정 세션에 저장된 값 1개 삭제
    invalidate() : 모든 세션을 해제 => logout
    isNew() : 새로 생성된 세션 여부 확인 => 장바구니
    setMaxInactiveInterval(int second) => 기간 설정
    => 1800초(30분)
    
    => 브라우저로 데이터 전송
       일회성 : request
       공유성 : session
    => 로그인 시에 사용자의 일부 정보를 저장
    => 댓글(ID) 참조
    ---------------------
    주의점
      1. 세션은 브라우저별로 개별 관리 
